"""
Strategy state machine matching Pine Script execution order.
...
Fill model:
- Stop buy entry: fill at max(open, stop) + slippage; triggered when high >= stop
- Limit sell exit: fill at limit - slippage (or open - slippage if gap up)
- Commission: percent of trade value per side
- Pending stop entries persist until filled or replaced (no cancel on invalidation)
"""
...
class StrategyState:
    ...
    def _compute_equity(self, current_price: float) -> float:
        ...
        openprofit = unrealized PnL (no commission deduction on open positions).
        netprofit = cumulative realized PnL from closed trades."""
        unrealized = sum(
    ...
    def process_bar(self, bar_index: int, bar, indicators,
        ...
        # ============================================================
        # STEP 2: Process pending LIMIT EXITS
        # ============================================================
        exits_to_remove = []
        fills_to_close = []
        ...
        # Pine: ta.crossover(high, bullBarConfirmationLevel)
        #   = prev_high <= prev_confirm AND current_high > current_confirm
        # Pine: ta.crossunder(low, bullBarInvalidationLevel)
        #   = prev_low >= prev_invalid AND current_low < current_invalid
        ...
        if current_opentrades > 0 and indicators.atr_value is not None:
            self.take_profit_level = (
                self.position_avg_price + indicators.atr_value * self.config.takeprofit_num_atr
            )
        else:
        ...
        # ============================================================
        # Pine calls strategy.exit() every bar for each entry
        # This updates the limit price each bar
