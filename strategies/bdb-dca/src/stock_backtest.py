"""
Bar-by-bar backtest engine for stock strategy with ATR stop loss.
"""
...
from .stock_strategy import StockStrategyState
from .risk import RiskConfig, RiskEngine


def run_stock_backtest(bars: list[Bar], config: StrategyConfig,
                       stoploss_atr_mult: float = 1.5,
                       takeprofit_atr_mult: float = 3.0,
                       risk_config: Optional[RiskConfig] = None) -> BacktestResult:
    """
    Run the stock backtest with ATR-based stop loss.

    Args:
        bars: Full bar list including warmup period
        config: Strategy configuration
        stoploss_atr_mult: ATR multiplier for stop loss (default 1.5)
        takeprofit_atr_mult: ATR multiplier for take profit (default 3.0)
    ...
    strategy = StockStrategyState(
        config,
        stoploss_atr_mult=stoploss_atr_mult,
        takeprofit_atr_mult=takeprofit_atr_mult,
    )
    ...
    # Attach risk engine and SL stats to result for inspection
    if risk_engine is not None:
        result.risk_engine = risk_engine
    result.sl_exits = strategy.sl_exits
